\chapter{The Method of Characteristics}{\label{ch:The Method of Characteristics}
    ...
    \section{Fundamentals}{\label{sec:MOC:Fundamentals}
        ...
        \source[mi][g][\loc]$ is given by
        \begin{equation}\label{eq:MOC:Source}
            \source[mi][g][\loc] \defined
                \left[\suml[\gprime]\suml[\ell=0][L](2\ell+1)\suml[n=-\ell][\ell]\SH\xss[\ell]\afluxmom(\loc) + \frac{\spect}{\keff}\suml[\gprime]\nufis\sflux[\gprime](\loc)\right].
        \end{equation}
        ...
        \subsection{Track-Length Renormalization}{\label{sec:MOC:Track-Length Renormalization}
            ...
            In this method, every monodirectional spatial integration should preserve the cell volume, i.e.
    ...
    \section{The Flat-Source Approximation}{\label{sec:MOC:FSA}
        ...
        \subsection{Derivation}{\label{ssec:MOC:FSA:Derivation}
            ...
            \begin{equation}\label{eq:MOC:FSA:Source Shape}
                \source[mi][g][\loc] \approx \srcF[mi] = \srcF + \suml[\ell=1][L](2\ell+1)\suml[n=-\ell][\ell]\SH\srcF[i,\ell][g,n]
            \end{equation}
            ...
            \begin{equation}\label{eq:MOC:FSA:Source Computation}
                \srcF[mi] =
                    \left[
                        \suml[\gprime]\suml[\ell=0][L]\suml[n=-\ell][\ell]\SH\xss[\ell]\afluxmom
                        + \frac{\spect}{\keff}\suml[\gprime]\nufis\sflux[\gprime]
                    \right],
            \end{equation}
            where the $\sflux[\gprime]$ is the region-averaged scalar flux, and $\afluxmom$ are the region-averaged higher-order angular moments of the flux.
            ...
            \begin{subequations}\label{eqs:MOC:FSA:Region-Averaged Flux Moments Definition}
                ...
                \begin{equation}\label{eq:MOC:FSA:Region-Averaged Angular Moments of Flux Definition}
                    \afluxmom[\ell][n][i][g] = \MOCIntegral{\SH\aflux[][g][]} = \frac{\fourpi}{V_i}\suml[m]\wt[m]\SH[\ell][n][\dirm]\suml[k]\nsegl\tA[mki]\MOCTrackIntegral{\aflux[][g][]}.
                \end{equation}
            \end{subequations}
        ...
        \subsection{Particle Conservation}{\label{ssec:MOC:FSA:Particle Conservation}
            ...
            \begin{subequations}\label{eqs:MOC:FSA:Anisotropic Constraints}
                \begin{equation}\label{eq:MOC:FSA:Quadrature Constraint}
                    \suml[m]\wt[m]\SH[\ell][n][\dirm]\SH[\ell'][n'] = \delta_{\ell\ell'}\delta_{nn'},
                \end{equation}
                and
            ...
            \begin{subequations}\label{eqs:MOC:FSA:Region-Averaged Flux Moments}
                ...
                \begin{equation}\label{eq:MOC:FSA:Region-Averaged Angular Moments of Flux}
                    \afluxmom[\ell][n][i][g] = \frac{\srcF[i,\ell][g,n]}{\xst} + \frac{\fourpi}{V_i\xst}\suml[m]\wt[m]\SH[\ell][n][\dirm]\suml[k]\tA[mki]\dflux,
                \end{equation}
                where
    ...
    \section{The Linear-Source Approximation}{\label{sec:MOC:LSA}
        \subsection{Overview}{\label{ssec:MOC:LSA:Overview}
            ...
            As part of this thesis work, significant effort has been spent on improvements to this moment-based \ac{LSA} in problems with near-void regions \cite{Fitzgerald2018}, and for the multiphysics problems that are of interest in the \ac{CASL} program \cite{Fitzgerald2019}.
        ...
        \subsection{Derivation}{\label{ssec:MOC:LSA:Derivation}
            ...
            \end{subequations}
            By performing a similar spatially linear expansion of the scalar flux,
            \begin{equation}\label{eq:MOC:LSA:Scalar Flux Expansion}
                \sflux(\loc) \approx \sfluxF + \loc\vdot\sfluxL,
            \end{equation}
            the sources linear expansion coefficients can be expressed as
            \begin{equation}\label{eq:MOC:LSA:Linear Source Computation}
                \srcL = \suml[\gprime]\xss[0]\sfluxL[i][\gprime] + \frac{\spect}{\keff}\suml[\gprime]\nufis\sfluxL[i][\gprime].
            \end{equation}

            In this \ac{LSA}, it is convenient to define the spatially linear source in terms of a cell-local coordinate system.
            ...
            Following the same approach as the \ac{FSMOC} derivation, in \cref{ssec:MOC:FSA:Derivation}, to compute the source the scalar flux region-averaged value, $\sfluxF$, the scalar flux expansion coefficients, $\sfluxL$, and the higher-order angular moments, $\afluxmom[\ell][n][i][g]$, are required.
            \begin{subequations}\label{eqs:MOC:LSA:Region-Averaged Flux Moments Definitions}
                The region-averaged scalar flux can be found using the same definition as previously,
                \begin{equation}\label{eq:MOC:LSA:Region-Averaged Scalar Flux Definition}
                    \sfluxF \defined \MOCIntegral{\aflux[][g][]} = \frac{\fourpi}{V_i}\suml[m]\wt\suml[k]\tA[mki]\nsegl\MOCTrackIntegral{\aflux[][g][]},
                \end{equation}
                the angular moments can be defined as
                \begin{equation}\label{eq:MOC:LSA:Region-Averaged Angular Moments of Flux Definition}
                    \afluxmom[\ell][n][i][g] \defined \MOCIntegral{\SH\aflux[][g][]} = \frac{\fourpi}{V_i}\suml[m]\wt\SH[\ell][n][\dirm]\suml[k]\tA[mki]\nsegl\MOCTrackIntegral{\aflux[][g][]}.
                \end{equation}
                In order to determine the spatial expansion coefficients of the scalar flux, \cref{eq:MOC:LSA:Scalar Flux Expansion} is operated on by $\MOCIntegral{\loc(\vdot)}$.
                Recognizing that this should be directly proportional to the angular flux moment $\MOCIntegral{\loc\aflux[][g][]}$, a system of equations is found
                \begin{equation}\label{eq:MOC:LSA:Moment to Expansion Coefficient}
                    \M\sfluxL = \MOCIntegral{\loc\aflux[][g][]},
                \end{equation}
                where
                \begin{equation}\label{eq:MOC:LSA:Geometric Moments}
                    \M \defined \MOCIntegral{\loc\loc^T}.
                \end{equation}
                The spatial angular flux moments, $\MOCIntegral{\loc\aflux[][g][]}$, are then defined as
                \begin{equation}\label{eq:MOC:LSA:Region-Averaged Spatial Flux Moments Definition}
                    \MOCIntegral{\loc\aflux[][g][]} = \frac{\fourpi}{V_i}\suml[m]\wt\suml[k]\nsegl\tA[mki]\left(\locIn\MOCTrackIntegral{\aflux[][g][]} + \dirm\MOCTrackIntegral{\nlen\aflux[][g][]} / \renorm[mi]\right).
                \end{equation}
            \end{subequations}
            ...
            \begin{subequations}\label{eqs:MOC:LSA:Region-Averaged Flux Moments}
                ...
                \begin{aequation}\label{eq:MOC:LSA:Region-Averaged Spatial Moments of Flux}
                    \MOCIntegral{\loc\aflux[][g][]}
                        = \M\frac{\srcL}{\xst}
                            &+ \frac{\fourpi}{V_i\xst}\suml[m]\wt\suml[k]\left(\dirm\segl\left[\frac{\dflux}{\segopt} - \afluxout\right]+ \locIn\dflux\right) \\
                            &+ \frac{\fourpi}{V_i\xst}\suml[m]\wt\suml[k]\frac{\srcF[mi]}{\xst}\left(\segopt\locCent + \dirm\segl\right).
                \end{aequation}
                where
        ...
        \subsection{Applications}{\label{ssec:MOC:LSA:Applications}
            ...
            As part of this work, a spatial mesh discretization study was performed on problems with multiphysics applications in mind \cite{Fitzgerald2019}; this is presented in more detail in \cref{sec:Results:2-D Linear Source}.
        }
    }
