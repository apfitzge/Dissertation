\chapter{MacroRay Three-Dimensional Ray-tracing Technique}{\label{ch:MacroRay}
  ...
  \section{MacroRay Technique}{\label{sec:MR:MacroRay Technique}
    ...
    \subsection{Macroband Technique}{\label{ssec:MR:Macroband Technique}
      ...
      In the macroband method, there are clearly regions which have more rays passing through them; in this case, it is the boron layer coating the fuel.
  ...
  \section{Results}{\label{sec:MR:Results}
    ...
    \subsection{C5G7}{\label{ssec:MR:C5G7}
      ...
      \subsubsection{C5G7 Benchmark: Unrodded}{\label{sssec:MR:C5G7:Unrodded}
        ...
        Although there is a 8 pcm difference between the two methods, this is not a significant difference.
      ...
      \subsubsection{C5G7 Benchmark: Rodded A}{\label{sssec:MR:C5G7:Rodded A}
        ...
        Again, in overall pin-power comparisons, the macroray method out performs the MRT method in all except the error in the maximum pin-power.
      ...
      \subsubsection{C5G7 Benchmark: Rodded B}{\label{sssec:MR:C5G7:Rodded B}
        ...
        The difference in eigenvalue error is likely primarily due to the fact that the \ac{MRT} benefits from cancellation of error while the macrorray generally does not.
        ...
        The macroray method also does not out perform the \ac{MRT} method in the number of pins within each uncertainty level of the reference.
  ...
  \section{Discussion on the Performance of MacroRay}{\label{sec:Discussion one the Performance of MacroRay}
    ...
    To determine where the performance was lacking, a miniaturized C5G7 unrodded case was profiled.
    This case was profiled without using \ac{CMFD} acceleration, and with 18 spatial domains, using both the \ac{MRT} and macroray methods.
    ...
    However, in the macroray calculation, only 22\% of the total time was spent on these loops.
    15.1 \% was spent on the conversion between ray fluxes and surface fluxes.
    ...
    While the conversion between ray and surface fluxes will never be zero, it is unlikely that the naive implementation of these routines in this work are near optimal.
  ...
  \section{Conclusions}{\label{sec:MR:Conclusions}
    The macroray method is a novel method which was motivated by previous studies on the macroband method that claimed significantly reduced number of rays with maintained accuracy.
    ...
    Additionally, for the shielded-box problem, this method demonstrated that it is able to act as more of a ``black box'' solver, allowing for users without deep knowledge of the method to use it.
    ...
    Additionally, the macrorray method consistently showed the highest errors in the upper 1/3 of the fuel in the C5G7 benchmark cases, where the pin-powers are lower.
    ...
    This work was is the first implementation of the macroray method for 3-D \ac{MOC} calculations.
